  ...
  return (
    <>
      ...
      >
        ...
        <Link to="/" className="flex items-center gap-2 relative z-50 hover:opacity-80 transition-opacity" onClick={() => setIsMobileMenuOpen(false)}>
          <img 
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navItems.map((item) => (
            <div key={item.name} className="relative group">
              <Link to={item.href} className={`flex items-center gap-1.5 text-xs tracking-[0.2em] uppercase font-medium transition-colors hover:text-[var(--color-cypress)] py-4 ${(isScrolled || isConsultation) ? 'text-[var(--color-ink)]' : 'text-white/90'}`}>
                {item.name}
              ...
              {item.submenu && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 w-56 bg-white border border-gray-100 shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 flex flex-col py-2 rounded-xl text-center">
                  {item.submenu.map((subItem) => (
                    <Link key={subItem.name} to={subItem.href} className="px-6 py-3 text-xs tracking-[0.1em] uppercase text-[var(--color-ink)] hover:bg-[var(--color-cloud)] hover:text-[var(--color-cypress)] transition-colors">
                      {subItem.name}
                    </Link>
                  ))}
                </div>
          ...
          <Link to="/consultation" className={`px-6 py-2.5 rounded-full text-xs tracking-[0.2em] uppercase font-medium transition-all ${(isScrolled || isConsultation) ? 'bg-[var(--color-walnut)] text-white hover:bg-[var(--color-forest)]' : 'bg-white/20 backdrop-blur-sm text-white hover:bg-white hover:text-[var(--color-walnut)]'}`}>
            Consultation
          </Link>
        </nav>
        ...
        <div className="flex flex-col px-6 gap-6 overflow-y-auto pb-12">
          {navItems.map((item) => (
            <div key={item.name} className="flex flex-col border-b border-gray-100 pb-4">
              <Link to={item.href} className="text-2xl font-serif text-[var(--color-walnut)]" onClick={() => !item.submenu && setIsMobileMenuOpen(false)}>
                {item.name}
              </Link>
              {item.submenu && (
                <div className="flex flex-col pl-4 mt-4 gap-4">
                  {item.submenu.map((subItem) => (
                    <Link key={subItem.name} to={subItem.href} className="text-xs tracking-[0.15em] uppercase text-[var(--color-ink)]/60 hover:text-[var(--color-cypress)]" onClick={() => setIsMobileMenuOpen(false)}>
                      {subItem.name}
                    </Link>
                  ))}
                </div>
          ...
          <Link to="/consultation" className="mt-8 px-6 py-4 bg-[var(--color-walnut)] text-white rounded-full text-xs tracking-[0.2em] uppercase font-medium w-full text-center" onClick={() => setIsMobileMenuOpen(false)}>
            Consultation
          </Link>
        </div>
      </div>
